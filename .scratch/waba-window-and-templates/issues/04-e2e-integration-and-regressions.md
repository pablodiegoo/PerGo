...
- [ ] All Go unit and integration tests pass with race detection (`go test ./... -race`).
- [ ] Template codegen (`templ generate`) succeeds without compilation errors or diff conflicts.
- [ ] WhatsApp Web (whatsmeow) and Telegram messaging paths operate without unintended customer service window blocks.
- [ ] Expiry error response formatting (`422 SESSION_WINDOW_EXPIRED`) remains compliant with API contracts.
