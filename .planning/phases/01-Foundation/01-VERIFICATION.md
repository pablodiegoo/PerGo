...
|----------|----------|--------|---------|
| `cmd/omnigo/main.go` | Composition root | ✓ VERIFIED | 207 lines, wires pgxpool, NATS, Echo, audit, auth, debug server, shutdown orchestrator |
| `cmd/omnigo/main_test.go` | Integration tests for boot/shutdown | ✓ VERIFIED | 4 tests: healthz, readyz, readyz-down, graceful-shutdown |
| `cmd/omnigo/auth_test.go` | Auth/identity tests | ✓ VERIFIED | 10 tests covering workspace, API key, middleware, encryption, tenant context |
| `cmd/omnigo/audit_test.go` | Audit pipeline tests | ✓ VERIFIED | 10 tests covering trace middleware, batch writer, structured logging |
| `cmd/omnigo/obs_test.go` | Observability/shutdown tests | ✓ VERIFIED | 7 tests covering pprof, expvar, orchestrator order/timeout/idempotency |
...
| `docker-compose.yml` | Docker Compose topology | ✓ VERIFIED | postgres:16-alpine, nats:2.10-alpine, omnigo service with health conditions |
...
|------|----|-----|--------|---------|
| cmd/omnigo/main.go | internal/api/handler/health.go | Import + RegisterRoutes | ✓ WIRED | HealthHandler created with pool + NATS, routes registered |
| cmd/omnigo/main.go | internal/api/middleware/auth.go | Import + AuthMiddleware(apiKeyRepo) | ✓ WIRED | AuthMiddleware wired globally on Echo |
| cmd/omnigo/main.go | internal/api/middleware/trace.go | Import + TraceMiddleware() | ✓ WIRED | TraceMiddleware wired globally before auth |
| cmd/omnigo/main.go | internal/platform/audit/batch.go | Import + NewWriter(pool, 5000, 2) | ✓ WIRED | AuditWriter created, registered in shutdown orchestrator |
| cmd/omnigo/main.go | internal/platform/obs/pprof.go | Import + StartDebugServer | ✓ WIRED | Debug server started on localhost, registered in shutdown |
| cmd/omnigo/main.go | internal/platform/shutdown/orchestrator.go | Import + NewOrchestrator | ✓ WIRED | All cleanup functions registered, Shutdown called on SIGTERM |
...
| internal/config/config.go | cmd/omnigo/main.go | Import + config.Load() | ✓ WIRED | Config loaded at composition root |
...
| INFRA-04 | 01-01 | Docker Compose deployment topology | ✓ SATISFIED | docker-compose.yml: postgres:16-alpine, nats:2.10-alpine, omnigo |
| INFRA-05 | 01-04 | Graceful shutdown (drain, flush, close) | ✓ SATISFIED | cmd/omnigo/main.go: signal.Notify + orchestrator.Shutdown(ctx); internal/platform/shutdown/orchestrator.go |
