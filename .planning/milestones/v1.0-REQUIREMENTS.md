...
- [ ] **INFRA-02**: PostgreSQL via pgx/v5 with dual-access model (pgxpool for PerGo queries + `pgx/v5/stdlib` bridge for whatsmeow `database/sql` and goose migrations)
- [ ] **INFRA-03**: Database migrations via goose with embedded SQL files (versioned, one file per migration)
- [ ] **INFRA-04**: Docker Compose deployment topology (pergo + postgres + nats)
