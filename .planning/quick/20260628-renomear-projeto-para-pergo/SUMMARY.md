...
status: complete
date: 2026-06-28
description: Global rename of the project from OmniGo to PerGo to prevent trademark and copyright conflicts.
---
...
2. **Directories & Files renamed**:
   - Renamed `cmd/omnigo` to `cmd/pergo`.
   - Renamed `docs/PRD OmniGo.md` to `docs/PRD PerGo.md`.
3. **Configuration & Environment variables**:
   - Updated `.air.toml` build output to `pergo`.
   - Updated `Makefile` targets, build/dev paths, and binary outputs.
   - Updated environment variable prefixes from `OMNIGO_` to `PERGO_` in all `.env` files, configs, and codebase references.
4. **Docs & Planning**:
   - Replaced all textual occurrences of `OmniGo` and `omnigo` with `PerGo` and `pergo` respectively in markdown documentation and plannings.
5. **Verification**:
