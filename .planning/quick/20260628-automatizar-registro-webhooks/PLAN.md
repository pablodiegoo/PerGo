...
## Tasks
1. **Meta Webhook Fallback Verification**: Add support in `waba_webhook.go` for verifying WABA webhook callbacks automatically using a predictable pattern: `pergo_verify_token_<workspace_id>`.
2. **Telegram Webhook Auto-Registration**:
   ...
   - If not HTTPS (local development), generate a predictable fallback secret token (`pergo_secret_token_<workspace_id>`) for local development manual testing.
